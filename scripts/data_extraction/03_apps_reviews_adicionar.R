...
apps <- read.csv("02_apps_details_temp.csv")
...
write.csv(dfr, "03_apps_reviews_temp.csv", row.names = TRUE)

appsDetallesOriginal <- read.csv("03_apps_reviews.csv")
appsDetallesOriginal <- subset(appsDetallesOriginal, select = -c(1))

appsDetallesNuevo <- read.csv("03_apps_reviews_temp.csv")
dfr <- rbind(appsDetallesOriginal, appsDetallesNuevo)
